...
public class AssertionT {
    // todo - разбить на классы по вложенным(для удобства поддержки)
...
//                Assert.assertTrue(wait.until(ExpectedConditions.attributeContains(webElement, "background-color", controlBorderColor)), "Ожидание появления [атрибута: свойства] --> background-color: " + controlBorderColor + " у элемента --> " + webElement);
        ...
        @Step("Изменение цветов [border-(bottom, left, right, top)-color: {colorBorder}] и [background-color: {colorBackground}] при даблклике")
        public static void checkMoveToHint(WebElement webElement, String title, String newHint) {
            ActionT.Move.moveToElement(webElement);
//            checkBorderColor(webElement, title);
//            Wait.attributeToBeWait(webElement, "title", title);
    ...
    public static class Sort {
        ...
        @Step("Проверка что после сортировки порядок элементов не совпадает (сортируется)")
        public static void twoClickListElementsIsNotEqualsByValue(String elementsList, String headerLocator) {
            ArrayList<String> beforeSortList = new ArrayList<>();
            ArrayList<String> afterSortList = new ArrayList<>();

            for (WebElement element : getDriverInstance().findElements(By.xpath(elementsList))) {
                beforeSortList.add(element.getAttribute("value"));
            }
            ...
            for (WebElement element : getDriverInstance().findElements(By.xpath(elementsList))) {
                afterSortList.add(element.getAttribute("value"));
            }
        ...
        @Step("Проверка что после сортировки порядок элементов не совпадает (сортируется)")
        public static void clickListElementsIsNotEquals2(WebElement header, String loc) {
            ArrayList<String> beforeSortList = new ArrayList<>();
            ArrayList<String> afterSortList = new ArrayList<>();
            List<WebElement> webElementsList = getDriverInstance().findElements(By.xpath(loc));
            for (WebElement element : webElementsList) {
                beforeSortList.add(element.getAttribute("value"));
            }
            ...
            List<WebElement> webElementsList2 = getDriverInstance().findElements(By.xpath(loc));
            for (WebElement element : webElementsList2) {
                afterSortList.add(element.getAttribute("value"));
            }
        ...
        @Step("Проверка что после сортировки порядок элементов не совпадает (сортируется)")
        public static void clickListElementsIsNotEquals3(WebElement webElement, String loc) {
            ArrayList<String> beforeSortList = new ArrayList<>();
            ArrayList<String> afterSortList = new ArrayList<>();
            List<WebElement> webElementsList = getDriverInstance().findElements(By.xpath(loc));
            for (WebElement element : webElementsList) {
...
//            ActionT.OneClick.click(headerLocator);
    ...
    public static class Element {
        ...
        public static void elementPresentOnPageAndHaveValue(WebElement webElement) {
            Assert.assertFalse("Поле осталось пустым", webElement.getAttribute("value").isEmpty());
        }
        ...
        public static void someElementsNotPresentOnPage(String locator) {
            Assert.assertFalse("Элементы не должны существовать!", getDriverInstance().findElements(By.xpath(locator)).size() > 0);
        }

        @Step("Проверка отсутствия локатора элемента")
        public static void someElementsNotPresentOnPage(List<WebElement> webElements) {
            WaitT.littleWait(300);
            Assert.assertFalse("Элементы не должны существовать!", webElements.size() > 0);
        }

        @Step("Проверка наличия локатора элемента")
        public static void someElementsPresentOnPage(String locator) {
            WaitT.littleWait(300);
            Assert.assertTrue("Элементы не существуют!", getDriverInstance().findElements(By.xpath(locator)).size() > 0);
        }
        ...
        public static void someElementsPresentOnPage(List<WebElement> webElements) {
            ...
            Assert.assertTrue("Элементы не существуют!", webElements.size() > 0);
        }
...
//            String listLocator = "//div[contains(@class, 'ScrollbarsCustom')]/ul/li/div";
            ...
            Assert.assertEquals("Выпадающий список эквивалентен эталону", elementsList, listMonthS);
        }
    ...
    public static class Calendar {
        ...
        public static void checkCalendar(WebElement dateBox, WebElement calendar) {
            ...
            Element.elementPresentOnPageAndHaveStringValue(dateBox,  "11.11.2124");
        }
        ...
        public static void checkCalendar(WebElement dateBoxLocator, WebElement calendarLocator, WebElement headerForm) {
            ...
            Element.elementPresentOnPageAndHaveAttribute(dateBoxLocator, "value", "11.11.2124");
        }
        ...
        private static void checkMonthList(By locator) {
            ...
            Assert.assertEquals("Выпадающий список с месяцами на эквивалентен эталону", month, listMonthS);
        }
...
//        }
    ...
    public static class CheckButton {
        ...
        public static void checkButtonStatus(WebElement webElement) {
            Assert.assertFalse("Кнопка не должна быть нажата", TurboNavigation.isElementPresent(webElement));
        }
        ...
        public static void enabledButton(WebElement webElement_Button, WebElement webElement_Pressed) {
            ...
            Assert.assertTrue("Кнопка не активировалась", TurboNavigation.isElementPresent(webElement_Pressed));
        }
        ...
        public static void offButton(WebElement webElement_Button, WebElement webElement_Pressed, WebElement webElement_Active) {
            ...
            Assert.assertTrue("Кнопка должна быть Активной", TurboNavigation.isElementPresent(webElement_Active));
        }

        @Step("Проверка состояния кнопки, что кнопка не активна")
        public static void checkButtonStatusOff(WebElement webElement_Active) {
            ActionT.OneClick.clickToHeader();
            Assert.assertFalse("Кнопка не должна быть Активной", TurboNavigation.isElementPresent(webElement_Active));
        }
    }
}