package ru.qa.property.components;

/**
 * Константы для открытия форм в разделе Автотестирование (примеры).
 * @author Evgeny Gribanov
 * @link egribanov@cbgr.ru
 * @since 08.07.2022 10:28
 */
public enum AutotestExample {

    // Автотест (примеры)
    ContextError("ContextError", "Тестирование контекстных ошибок"),
    DialogsTests("DialogsTests", "Пример тестирования диалогов");//Диалоги

    public final String formName;
    public final String title;

    /** ! возврат data-testid обязателен, по нему происходят переходы по формам  */
    @Override
    public String toString() {
        return title;
    }

    AutotestExample(String formName, String title) {
        this.formName = formName;
        this.title = title;
    }

    public String getFormName() {
        return formName;
    }
}
